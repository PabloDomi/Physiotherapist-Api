...
class ExerciseAnalyzer:
    ...
    def calculate_velocity(self):
        """
        Calcula la velocidad en los ejes x, y, z y añade estas velocidades
        como nuevas columnas en el DataFrame.
        """
        ...
        for landmark in self.df["landmark"].unique():
            positions_x = self.df.loc[
                self.df["landmark"] == landmark, "x"
            ].values
            positions_y = self.df.loc[
                self.df["landmark"] == landmark, "y"
            ].values
            positions_z = self.df.loc[
                self.df["landmark"] == landmark, "z"
            ].values
    ...
    def calculate_smooth_velocity(self):
        """
        Calcula la velocidad suavizada en los ejes x, y, z y añade estas
        velocidades suavizadas como nuevas columnas en el DataFrame.
        """
        ...
        for landmark in self.df["landmark"].unique():
            ...
            vz_smooth = savgol_filter(
                vz,
                window_length=self.window_length,
                polyorder=self.polyorder
            )

            # Insertar las velocidades suavizadas en las posiciones
            # correspondientes en el array temporal
    ...
    def calculate_peaks(self):
        """
        Calcula los picos en los datos de velocidad suavizada y los almacena
        en un diccionario.
        """
        ...
        for landmark in self.df["landmark"].unique():
            ...
            for axis in ["vx", "vy", "vz"]:
                ...
                data = self.df.query(f"landmark == {landmark}")[
                    f"{axis}_smooth"
                ].tolist()
    ...
    def apply_group_peaks_to_dict(self):
        """
        Aplica la función _group_peaks a los picos del diccionario y almacena
        los resultados en un nuevo diccionario.
        """
        grouped_dict = {}
        ...
        for key, value in self.peaks_dict.items():
            ...
            for subkey, peaks in value.items():
                if len(peaks) > 0:  # Asegurar que no es un array vacío
                    grouped_dict[key][subkey] = self._group_peaks(
                        peaks,
    ...
    def create_intervals(self):
        """
        Crea intervalos a partir de los picos agrupados y los almacena en un
        diccionario.
        """
        intervals_dict = {}
    ...
    def analyze_exercise(self):
        ...
        for i, (interval, peaks) in enumerate(zip(self.total_intervals, self.total_peaks)):
            ...
            # Convierte la tupla a cadena antes de usarla como clave
